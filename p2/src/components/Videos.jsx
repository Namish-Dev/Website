    ...
    <Stack direction={['column','row']} h={'100vh'}>
      <VStack w={'full'}>
        ...
        <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'}>
          ...
          <Text>This is an description of the videos so plz dont read it and waste your time </Text>
        </VStack>
      </VStack>
      <VStack w={['full','xl']} alignItems={'stretch'} p={'8'} spacing={'8'} overflowY={'auto'}>
        {videosArr.map((item,index)=>(
          <Button colorScheme='purple' variant={'ghost'} onClick={()=>{setVideoSrc(item); setHeading(index+1)}}>Videos {index+1}</Button>
        ))}
      </VStack>
    </Stack>
  )
